  ...
  <div class="favorite-card" v-click-away="closeContent">
    <div class="favorite-card__header">
      <div class="favorite-card__header-col">
        ...
        ></r-checkbox>
        <h4 class="favorite-card__source">{{ parsource.data_source }}</h4>
        <p class="favorite-card__favorite">
          {{ parsource.parsers.length || 0 }}/{{ parsource.find }} в избранном
        </p>
        <p class="favorite-card__date">{{ prettyDate(parsource.date) }}</p>
      </div>
      <div class="favorite-card__header-col">
        <r-button
          color="bordered"
