    ...
    <template v-else-if="userRate?.id && !isLoading">
      <div class="the-rates__stats">
        ...
        <div class="the-rates__stats-buttons">
          <p class="the-rates__stats-status" v-if="pendingForCapture">
            Ожидание подтверждения
          </p>
          <template v-else>
            <r-button text="Перейти к оплате" :disabled="userRate.is_active" />
...
export default {
  ...
  computed: {
    ...
    pendingForCapture() {
      if (this.userRate.status === "payment.waiting_for_capture") {
        return true;
      }
      return false;
    },
  ...
  methods: {
    ...
    select_rate(rate_id) {
      ...
      switch (rate_id) {
        case 1: {
          window.open(`${this.baseURL}/api/pay/${rate_id}`, "_blank").focus();
          break;
        }
        case 2: {
          window.open(`${this.baseURL}/api/pay/${rate_id}`, "_blank").focus();
        ...
        case 3: {
          window.open(`${this.baseURL}/api/pay/${1}`, "_blank").focus();
          break;
        }
        case 4: {
          window.open(`${this.baseURL}/api/pay/${2}`, "_blank").focus();
          break;
        }
      }
    },
...
.the-rates {
  ...
  &__help {
    background: url("/public/img/cabinet/message.png") center / cover no-repeat;
    display: flex;
    flex-direction: column;
    gap: 3rem;
